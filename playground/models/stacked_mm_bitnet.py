...
class DynamicPositionBias(nn.Module):
    def __init__(self, dim, *, heads, depth, log_distance=False, norm=False):
        ...
        self.mlp.append(
            Sequential(
                BitLinear(1, dim), nn.LayerNorm(dim) if norm else None, nn.SiLU()
            )
        )
        ...
        for _ in range(depth - 1):
            self.mlp.append(
                Sequential(
                    BitLinear(dim, dim), nn.LayerNorm(dim) if norm else None, nn.SiLU()
                )
            )

        self.mlp.append(BitLinear(dim, heads))
...
class GLU(nn.Module):
    def __init__(self, dim_in, dim_out, activation: Callable, mult_bias=False):
        super().__init__()
        self.act = activation
        self.proj = BitLinear(dim_in, dim_out * 2)
...
class FeedForward(nn.Module):
    def __init__(
        ...
        if glu:
            ...
            project_in = nn.Sequential(BitLinear(dim, inner_dim), activation)

        self.ff = Sequential(
            project_in,
            nn.LayerNorm(inner_dim) if post_act_ln else None,
            nn.Dropout(dropout),
            BitLinear(inner_dim, dim_out),
        )
...
class Attention(nn.Module):
    def __init__(
        ...
        self.to_q = BitLinear(dim, q_dim)
        self.to_k = BitLinear(dim, k_dim)

        # shared key / values, for further memory savings during inference
        assert not (
            shared_kv and value_dim_head != dim_head
        ), "key and value head dimensions must be equal for shared key / values"
        self.to_v = BitLinear(dim, v_dim) if not shared_kv else None

        # relations projection from tp-attention
        self.to_r = BitLinear(dim, v_dim) if tensor_product else None

        # add GLU gating for aggregated values, from alphafold2
        self.to_v_gate = None
        if gate_values:
            self.to_v_gate = BitLinear(dim, out_dim)
        ...
        if gate_value_heads:
            self.to_v_head_gate = BitLinear(dim, heads)
        ...
        self.to_out = (
            nn.Sequential(BitLinear(out_dim, dim * 2), nn.GLU())
            if on_attn
            else BitLinear(out_dim, dim)
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.patch_to_embedding = nn.Sequential(
            nn.LayerNorm(patch_dim), BitLinear(patch_dim, dim), nn.LayerNorm(dim)
        )
        ...
        self.mlp_head = (
            BitLinear(dim, num_classes) if exists(num_classes) else nn.Identity()
        )
...
class TransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.project_emb = BitLinear(emb_dim, dim) if emb_dim != dim else nn.Identity()
        self.attn_layers = attn_layers
...
model = TransformerWrapper(
    num_tokens=20000, max_seq_len=1024, attn_layers=Decoder(dim=512, depth=12, heads=8)
)
