...
class GatedXDenseBlock(nn.Module):
    def __init__(
        self,
        dim,
        heads,
        ...
        # Feedforward
        self.ffw = znn.SimpleFeedForward(dim, dim, dropout)

        # Self Attention
        self.self_attn = ParallelTransformerBlock(
            dim=dim,
            dim_head=dim_head,
            heads=heads,
        )
    ...
    def forward(self, x, y):
        # X is the text, Y is the image
        # Project the queries, keys, and values from text and images
        ...
        # cross attention
        attn = self.cross_attn(y, x)

        # gating
        gated = self.gate(attn) + q

        # Feedforward
        x = self.ffw(x)

        # Gating2
        gated2 = self.gate(x) + gated

        # Self Attention
        self_attn = self.self_attn(x)

        # Add the gated output to the self-attention output
        x = gated2 + self_attn

        # Feedforward
        x = self.ffw(x) + self_attn
...
def Flamingo(*, dim, num_tokens, depth, dim_head=64, heads=8, ff_mult=4):
    net = nn.Sequential(
