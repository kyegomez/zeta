...
git clone https://github.com/Agora/zeta.git
cd zeta
pip install -e .
...
We use a [streaming dataloader](https://github.com/Agora/infinibatch) to read the data on-the-fly from the disk. It requires the data sharded into multiple small files (e.g. 10K lines per file), as well as a JSON file to contain some meta data and the paths to these files.
...
You can quickly get started with our processed vocabulary files: [sentencepiece.bpe.model](https://publicmodel.blob.core.windows.net/zeta/vocab/sentencepiece.bpe.model) and [dict.txt](https://publicmodel.blob.core.windows.net/zeta/vocab/dict.txt). Note that this vocabulary is English-only with 64K tokens. To train a new `sentencepiece.bpe.model` on your own data, please refer to the [SentencePiece](https://github.com/google/sentencepiece) repo. With the sentecepiece model and the installed `sentencepiece` library, you can extract the `dict.txt` file from it by
```
