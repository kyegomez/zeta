...
def decoupled_optimizer(
    ...
    # Create an empty dictionary called param_dict to store the model's named
    # parameters.
    param_dict = {}
    # Iterate over the model's named parameters and populate the param_dict
    ...
    # Create an empty list to store the names of the LayerNorm and Embedding
    # layer weights with no weight decay.
    no_decay = []
    ...
    # Create an empty list to store the names of the Linear layer weights with
    # weight decay.
    decay = []

    # Iterate through the named modules of the model.
    for module_name, module in model.named_modules():
        # Check if the current module is an instance of the desired type
        # (torch.nn.Linear).
        for ndim in [torch.nn.Linear]:
            if isinstance(module, ndim):
                # If the module is an instance of torch.nn.Linear, append its
                # name with a ".weight" suffix to the decay list.
    ...
    # The no_decay_param list contains the parameters that should not have
    # weight decay applied, excluding the 'to_logits.weight' parameter.

    # Create an empty list called decay_param to store the parameters with
    # weight decay.
    decay_param = []
    ...
    # Iterate over the decay list, which contains the names of the parameters
    # with weight decay.
    for param in decay:
        # Check if the current parameter is not 'to_logits.weight'.
        # Append the corresponding parameter from param_dict to the decay_param
        # list.

        if param != exclude_param:
            decay_param.append(param_dict[param])

    # Create an empty list called no_decay_param to store the parameters
    # without weight decay.
    no_decay_param = []

    # Iterate over the no_decay list, which contains the names of the
    # parameters without weight decay.
    for param in no_decay:
        # Append the corresponding parameter from param_dict to the
        # no_decay_param list.
    ...
    # The second dictionary has the no_decay_param list and a weight_decay
    # value of 0.0.
    grouped_params = [
