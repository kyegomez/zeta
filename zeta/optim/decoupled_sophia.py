...
class SophiaG(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        step_is_tensor = (
            len(state_values) != 0) and torch.is_tensor(
            state_values[0]['step'])
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    state['step'] = torch.zeros(
                        ...
                        dtype=torch.float,
                        device=p.device) if self.defaults['capturable'] else torch.tensor(0.)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.defaults['capturable']:
                    bs = torch.ones(
                        (1,), dtype=torch.float, device=p.device) * bs
    ...
    def _single_tensor_sophiag(self, params: List[Tensor],
        ...
        for i, param in enumerate(params):
            ...
            if capturable:
                ...
                ratio = (exp_avg.abs() / (rho * bs *
                         hess + 1e-15)).clamp(None, 1)
