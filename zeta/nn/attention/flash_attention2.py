    ...
    def forward(ctx, q, k, v, mask, causal, q_bucket_size, k_bucket_size):
        ...
        if not exists(mask):
            ...
            mask = (
                ((mask,) * num_row_tiles)
                if mask.shape[-2] == 1
                else mask.split(q_bucket_size, dim=-2)
            )
            mask = tuple(
                ((row_mask,) * num_col_tiles)
                if row_mask.shape[-1] == 1
                else row_mask.split(k_bucket_size, dim=-1)
        ...
        for ind, (qc, oc, row_mask, row_sums, row_maxes) in enumerate(row_splits):
            ...
            col_splits = zip(
                k.split(k_bucket_size, dim=-2), v.split(k_bucket_size, dim=-2), row_mask
            )
...
class FlashAttentionTwo(BaseAttention):
    ...
    def forward(
        ...
        if self.parallel:
            # Split the input data into chunks and move each chunk to the
            # correct GPU
