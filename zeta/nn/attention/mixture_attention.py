...
class Attention(nn.Module):
    ...
    def forward(
        ...
        q, k, v = map(
            lambda t: rearrange(
                t, 'b (g h d) n -> b g h n d', h=h, g=g), (q, k, v))
        ...
        nk, nv = map(
            lambda t: repeat(
                t, 'g h 1 d -> (b g) h 1 d', b=b), self.null_kv)
...
class MixtureOfAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.null_routed_token = nn.Parameter(
            torch.randn(1, 1, dim)) if not local_attn else None
    ...
    def forward(
        ...
        num_routed_key_values = default(
            num_routed_key_values,
            self.num_routed_key_values)

        is_cross_attn = exists(context)
        assert not (exists(self.local_attn)
                    and is_cross_attn), 'cannot do cross attention with local attention'
        ...
        if exists(rotary_emb):
            ...
            q_rotary_emb = rotary_emb[query_indices] if exists(
                query_indices) else rotary_emb
            k_rotary_emb = rotary_emb[kv_indices] if exists(
                kv_indices) else rotary_emb
...
class MixtureOfAutoregressiveAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        routed_window_size = default(
            routed_window_size, local_attn_window_size)
    ...
    def forward(
        ...
        if exists(rotary_emb):
            ...
            windowed_rotary_emb = rearrange(
                rotary_emb, '(n w) d -> n w d', w=w)
            ...
            k_rotary_emb = rotary_emb[kv_indices] if exists(
                kv_indices) else rotary_emb[:context.shape[-2]]
            rotary_emb = (q_rotary_emb, k_rotary_emb)
