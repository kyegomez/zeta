...
class LowerTriangularMaskWithTensorBias(LowerTriangularMask):
    ...
    def materialize(
        ...
        return (
            super().materialize(shape, dtype=dtype, device=device) + self._bias
        )
...
class BlockDiagonalMask(AttentionBias):
    ...
    def from_tensor_lists_qkv(
        ...
        "BlockDiagonalMask", torch.Tensor, torch.Tensor, Optional[torch.Tensor]
    ]:
        ...
        return (
            block_diag,
            torch.cat(
                [x.reshape([1, -1, *x.shape[2:]]) for x in tensors_q], dim=1
            ),
            torch.cat(
                [x.reshape([1, -1, *x.shape[2:]]) for x in tensors_k], dim=1
            ),
            (
                torch.cat(
                    [x.reshape([1, -1, *x.shape[2:]]) for x in tensors_v], dim=1
                )
